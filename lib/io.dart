...
class CheckOrdinatesFilter implements CoordinateSequenceFilter {
  ...
  CheckOrdinatesFilter(this.checkOrdinateFlags) :
...
class WKTWriter {
  ...
  static const int OUTPUT_DIMENSION = 2;
...
class WKTReader {
  ...
  static const String NAN_SYMBOL = "NaN";
  ...
  /// is supported.
  static const bool ALLOW_OLD_JTS_COORDINATE_SYNTAX = true;
  ...
  /// is supported.
  static const bool ALLOW_OLD_JTS_MULTIPOINT_SYNTAX = true;
  ...
  WKTReader() : this.withFactory(GeometryFactory());
...
// read close token if it was opened here
  ...
  CoordinateSequence getCoordinateSequenceTryParen(
    ...
    if (getNextEmptyOrOpener(tokenizer) == EMPTY) {
      return this.csFactory.createWithSize(0, toDimension(ordinateFlags),
  ...
  CoordinateSequence mergeSequences(
    ...
    if (sequences == null || sequences.isEmpty) {
      return csFactory.createWithSize(0, toDimension(ordinateFlags));
    }
    ...
    CoordinateSequence sequence = this.csFactory.createWithSize(
        sequences.length,
  ...
  Coordinate getPreciseCoordinate(WKTTokenizer tokenizer) {
    ...
    precisionModel.makePreciseFromCoord(coord);
    return coord;
  }
...
//        return "End-of-Line";
  ...
  Geometry readGeometryTaggedTextWithOpts(
    ...
    // ordinate bit-flag.
    try {
      csFactory.createWithSize(0, toDimension(ordinateFlags),
          ordinateFlags.contains(Ordinate.M) ? 1 : 0);
    } catch (e) {
  ...
  LineString readLineStringText(
    ...
    return geometryFactory
        .createLineStringFromSeq(getCoordinateSequence(tokenizer, ordinateFlags));
  }
  ...
  LinearRing readLinearRingText(
      WKTTokenizer tokenizer, List<Ordinate> ordinateFlags) {
    return geometryFactory
        .createLinearRingFromCoordSeq(getCoordinateSequence(tokenizer, ordinateFlags));
  }
  ...
  MultiPoint readMultiPointText(
      WKTTokenizer tokenizer, List<Ordinate> ordinateFlags) {
    return geometryFactory.createMultiPointFromCoordSeq(getCoordinateSequenceTryParen(
