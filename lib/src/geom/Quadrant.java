...
class Quadrant 
{
   ...
   * 
   * @throws ArgumentError if the displacements are both 0
   */
  static int quadrant(double dx, double dy)
  {
    if (dx == 0.0 && dy == 0.0)
      throw new ArgumentError("Cannot compute the quadrant for point ( "+ dx + ", " + dy + " )" );
    if (dx >= 0.0) {
      if (dy >= 0.0)
        return NE;
      else
   ...
   * 
   * @throws ArgumentError if the points are equal
   */
  static int quadrant(Coordinate p0, Coordinate p1)
  {
    if (p1.x == p0.x && p1.y == p0.y)
      throw new ArgumentError("Cannot compute the quadrant for two identical points " + p0);
