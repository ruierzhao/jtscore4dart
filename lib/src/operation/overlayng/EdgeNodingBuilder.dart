...
import 'package:jtscore4dart/src/noding/ValidatingNoder.dart';
import 'package:jtscore4dart/src/noding/snapround/SnapRoundingNoder.dart';

import 'Edge.dart';
import 'EdgeMerger.dart';
import 'EdgeSourceInfo.dart';
import 'LineLimiter.dart';
import 'OverlayUtil.dart';
import 'RingClipper.dart';
...
class EdgeNodingBuilder {
   ...
   */
  /**private */ static const int MIN_LIMIT_PTS = 20;

  /**
   * Indicates whether floating precision noder output is validated.
   */
  /**private */ static const bool IS_NODING_VALIDATED = true;
  ...
  /**private */
  static Noder createFloatingPrecisionNoder(bool doValidation) {
...
//  /**private */bool[] hasEdges = new bool[2];
  /**private */ List<bool> hasEdges = List.filled(2, false, growable: false);
   ...
   */
  List<Edge> build(Geometry geom0, Geometry geom1) {
    add(geom0, 0);
    add(geom1, 1);
  ...
  /**private */ List<Edge> node(List<NodedSegmentString> segStrings) {
    ...
    Iterable<SegmentString> nodedSS = noder.getNodedSubstrings();
    List<Edge> edges = createEdges(nodedSS);
    return edges;
  }

  /**private */ List<Edge> createEdges(Iterable<SegmentString> segStrings) {
  ...
  /**private */ void addGeometryCollection(
    ...
    for (int i = 0; i < gc.getNumGeometries(); i++) {
      ...
      if (g.getDimension() != expectedDim) {
        throw new ArgumentError("Overlay input is mixed-dimension");
      }
      add(g, geomIndex);
    }
  }
  ...
  /**private */ static int computeDepthDelta(LinearRing ring, bool isHole) {
     ...
     */
    bool isCCW = Orientation.isCCWFromCS(ring.getCoordinateSequence());
    /**
  ...
  /**private */ void addLine(LineString line, int geomIndex) {
    ...
    if (isToBeLimited(line)) {
      ...
      for (List<Coordinate> pts in sections) {
        _addLine(pts, geomIndex);
      }
    } else {
      List<Coordinate> ptsNoRepeat = removeRepeatedPoints(line);
      _addLine(ptsNoRepeat, geomIndex);
    }
  }

  /**private */ void _addLine(List<Coordinate> pts, int geomIndex) {
    /**
     * Don't add edges that collapse to a point
     */
    if (pts.length < 2) {
